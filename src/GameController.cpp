...
void GameController::start(State state) {
    ...
    while(running) {
        ...
        if (state.turn % 2 ==0 ? Move move = currentPlayer.getMove(state, availableMoves,lastBlackMove) : Move move = currentPlayer.getMove(state, availableMoves, lastWhiteMove);)
