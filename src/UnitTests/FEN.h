...
TEST_CASE("FEN conversion: Simple move sequence", "[fen]") {
    ...
    REQUIRE(state == State("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"));

    // Move white Pawn
    state = MoveUtil::executeMove(state, { {4,1}, {4,3} });
    REQUIRE(state == State("rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq e3 0 1"));

    // Move black pawn
    state = MoveUtil::executeMove(state, { {2,6 }, {2,4} });
    REQUIRE(state == State("rnbqkbnr/pp1ppppp/8/2p5/4P3/8/PPPP1PPP/RNBQKBNR w KQkq c6 0 2"));

    // Move White Knight
    state = MoveUtil::executeMove(state, { {6,0}, {5,2} });
    REQUIRE(state == State("rnbqkbnr/pp1ppppp/8/2p5/4P3/5N2/PPPP1PPP/RNBQKB1R b KQkq - 1 2"));
}
...
TEST_CASE("FEN conversion: Castling", "[fen]") {
    ...
    REQUIRE(state == State("rnbqkbnr/3ppppp/ppp5/8/8/6PN/PPPPPPBP/RNBQ1RK1 b kq - 0 4"));
}
